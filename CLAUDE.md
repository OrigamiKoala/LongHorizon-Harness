...
- `planner.py`: Windowed planner operating on unit labels/token counts (no source content). Enforces depth cap (4) and node cap (400). `plan_level`/`build_tree`/`_render_slice` accept `unit_summary_for` (capped explorer findings) and `on_reasoning`. §D28: `recurse` rewrites the model's `depends_on` edges onto final path-prefixed leaf ids, re-points edges onto a recursed sibling at its descendant leaves (conservative all-children re-point), and runs a post-build sweep so a plan phase can never ship an unloadable `tree.json`.
...
**818 tests, all passing.** *(2026-08-13: 855 after the claude/codex backend port — `test_agent_worker.py`, `test_backends_claude_codex.py`; 905 after the backend-override work — `test_backend_toggle.py`. 2026-08-14: 940 after the `provider.json` schema rewrite (§12) and an in-flight gptme provider-selection feature landed the same day; 947 after the new-run modal backend+model fix; 949 after `resolve()`'s opencode-backend-model routing — `test_provider_config.py`. 2026-08-15: 954 after §D12/§D13 — PDF extraction through the dashboard, over-limit-line survival, logdir dedupe; 979 after §D28 — planner `depends_on` rewriting — `test_v2_planner.py`.)*

980 after §D29 — task tree orphaned intermediate folders — `test_dashboard_server.py`.)*

982 after §D30 — reviewer defect over-length — `test_v1_reviewer_fanout.py`.)*

984 after §D31 — in-place retries never regenerate — `test_pipeline_prompts.py`.)*

985 after §D32 — persistent blocked-nodes banner — `test_dashboard_server.py`.)*
...
## §D28 Recursive planning ships dangling `depends_on` edges (P1) — FIXED 2026-08-15

`v2/planner.py:build_tree`'s `recurse` stored the model's `depends_on` candidate ids verbatim while final node ids are **path-prefixed leaves** (candidate `opening` under path `c01` becomes node `c01.opening`), and a **recursed candidate never becomes a node at all** — its children replace it. Both left a dangling edge in the saved `tree.json`, and the next `TaskTree.load` rejected the whole run with `TreeValidationError: node 'c01.kinetic-motion' depends_on unknown node 'opening'`. Observed live on a 12-chapter T3 textbook study-guide run (`rec178681445199a5ed`, plan phase): six dangling edges across three levels — `c01.kinetic-motion -> ['opening']`, `c01.isotherms -> ['kinetic-motion']`, `c03.simple-mixtures-thermo.relation-follows-use -> ['molality-mole-fraction-relation']`, `c03.simple-mixtures-thermo.simple-mixtures-molar-gibbs -> ['relation-follows-use']`, `c04.chem-worked-solution.zeff-trend-mg-noble -> ['calc-part-b-integral']` (each a bare candidate id whose final node exists prefixed), and `c08 -> ['c02']` (a sibling recursed into `c02.protein_stability_and_setup` + `c02.gas_thermal_contact`, leaving no `c02` node at all). The §M2 flat-plan test never caught it — single level, no prefixes, no recursion — and the driver saves `build_tree`'s output without a post-save load, so the invalid tree shipped to disk and the failure surfaced only when `_phase_pilot` (and then driver.py:561's report `_load_tree`) loaded it — a recoverable phase failure turned into `driver_crashed`.

**Fix (in `recurse`, two passes per level):** (1) classify every candidate — leaf candidates reserve budget and final ids (`unique_id`), non-leaves recurse and return their descendant leaf ids; (2) add the leaves with edges rewritten — a dep on a leaf sibling re-points at the sibling's final id (`kinetic-motion -> opening` becomes `-> c01.opening`), a dep on a recursed sibling re-points at **every** descendant leaf (`c08 -> c02` becomes `-> [c02.protein_stability_and_setup, c02.gas_thermal_contact]` — conservative: preserves the ordering the model asked for without guessing which child holds the content), anything unresolvable (e.g. a cap-dropped sibling) is dropped with a `planner_dep_dropped` event; each rewrite emits `planner_dep_rewritten {dep, to, reason}`. A final belt-and-suspenders sweep after the whole recursion drops any residual edge not naming an existing node, so a plan phase can never ship an unloadable `tree.json` regardless of model pathology (duplicate ids, renames). Cross-level edges stay impossible: sanitization still runs per-level and deeper levels' edges point within their own subtree, so the rewrite cannot create a cycle. Failing-first: `DependsOnRewriteTest` in `test_v2_planner.py` — `test_depends_on_rewritten_to_prefixed_final_ids` (leaf-sibling dep round-tripped through `TaskTree.load`), `test_depends_on_onto_recursed_sibling_repoints_at_descendant_leaves` (the `c08 -> c02` shape), `test_depends_on_onto_recursed_sibling_works_at_deeper_levels` (transitive, 3 levels) — all three failed with the raw-edge behavior before the fix. The live run's tree was repaired in place with the same mapping (bare id → `<path>.<id>`; recursed id → all descendants; backup at `tree.json.bak`) and loads clean; the run resumes at pilot.

## §D29 The task tree orphans intermediate folders (P1) — FIXED 2026-08-15

`dashboard/static/app.js:buildNodeTreeIndex` created a folder key for every segment of every node id, but only **full node ids** were attached to their parent's `children` — a folder whose exact id had no node row (e.g. `c03.simple-mixtures-thermo` when only `c03.simple-mixtures-thermo.<leaf>` nodes exist) was unreachable, and **every subtree deeper than one level rendered as an empty folder** in the task tree. Observed live on the §D28 T3 textbook run (`rec178681445199a5ed`): c03–c07 and c10–c12 all showed `▾` with no children despite two c04 leaves already `passed` — the operator watched a just-finished `c04.chem-worked-solution.addition-configs-comparison.addition-contending-configs` episode with nothing under c04 in the tree. c01/c02 worked only because their children were direct 2-segment ids, which is why the bug went unnoticed through §C4's split-parent rendering work. **Fix:** each newly-created segment (folder or leaf) attaches to its parent *inside* the prefix loop (`parts.slice(0, i - 1)`); the post-loop push became redundant (a node's own key is attached at creation, or pre-existed as an attached folder) and was removed. Failing-first: `test_tree_index_attaches_intermediate_folders` (source-text regression asserting the folder-attach line — this repo has no jsdom harness, §9.4's `node --check` is the only JS gate); verified against the live run's `tree.json` that every c01–c12 subtree now renders, and `node --check` passes.

## §D30 A reviewer defect over the schema's `maxLength` kills the run (P1) — FIXED 2026-08-15

`v1/reviewer.py`'s `VERDICT_SCHEMA` capped `defect` at 300 chars, and when the host rejects `response_format` (the A4-1 latch), `complete_json` enforces the schema *harness-side* — 3 failed reprompts raise `ProviderError("structured output failed after 3 attempts: $.items[0].defect: longer than maxLength 300")`, which `_run_phase` correctly treats as deterministic (§E10) and does **not** retry, so the exception propagates out of `review_node` → `run_round_loop` → the whole run parks with a phase failure. Observed live: real "scoped, located" defects with math notation (`"…the evaluation of the non-trivial Gaussian integral ∫₋∞^∞ e^(−mvx²/2kT)dvx that yields it is never shown."` ≈ 330 chars) repeatedly exceeded 300, and the reviewer on an `awaiting_review` node killed the run instead of recording a verdict. Two fixes, both in `reviewer.py`: (1) the primary cap moves 300 → `DEFECT_MAXLENGTH = 400` (fits the observed natural length of a located defect carrying notation; still small, invariant 7); (2) `_call_reviewer` catches a `ProviderError` whose message names `maxLength` and retries **once** against a deep copy of the schema with `RELAXED_DEFECT_MAXLENGTH = 600` — the harness revising its own constraint, same degrade-don't-die convention as A4-1's response_format fallback; anything else propagates unchanged. The verdict then lands, the audit record carries the full defect, and the node transitions normally. Failing-first: `DefectOverLengthTest` in `test_v1_reviewer_fanout.py` — `test_over_length_defect_falls_back_to_relaxed_schema` (first strict call raises, retry succeeds with the 600 schema, defect survives intact) and `test_unrelated_provider_error_still_propagates` — both failed before the fix. The parked run recovers on resume: the `awaiting_review` node's verdict is uncached, so execute re-reviews it with the fixed schema.

## §D31 In-place retries burn attempts on regenerations (P1) — FIXED 2026-08-15

`pipeline/prompts.py` framed a retry by attempt count — patch on attempt 2, **regenerate on attempt 3+**. The mid-series rewrite is the worst of both: it starts a fresh artifact that must re-clear the same gates, throws away the scoped defects' patch target, and the observed retries got *faster* each time (105 s → 46 s → 41 s), not more thorough — a rewrite framing invites a shorter, less careful pass. **Fix:** an in-place retry is now always patch-framed ("make the MINIMAL change… fix it in place", with the prior artifact inlined per A6-5); regenerate framing fires only for an **operator redispatch** — the node exhausted its series and `dashboard/state.py`'s redispatch job reset it to `pending`/`attempts=0` while stamping `last_defect` with the `"redispatch requested by operator: "` prefix, which is the marker the framing now matches (the pre-reset attempt count isn't persisted, so the operator's explicit intervention is the fresh-start judgment call). A redispatch regenerate gets no prior-artifact inline — a from-scratch rewrite must not be tempted to patch (`_prior_attempt_artifact`'s own comment already treated inlining as patch-oriented). A failure *within* the redispatched series falls back to patch framing normally (reviewer defects, attempts=1). Failing-first: `test_attempt_three_still_uses_patch_framing`, `test_operator_redispatch_uses_regenerate_framing` (no MINIMAL, no inline), `test_redispatch_failure_returns_to_patch_framing` — the first failed against the old attempt-count framing; the module docstring and `test_pipeline_prompts.py`'s header were updated to match.

## §D32 Blocked nodes are invisible while the run is still executing (P2) — FIXED 2026-08-15

`node_blocked`/`run_escalated` cards fire only when the round loop parks (no ready nodes, nothing in flight) — while other leaves are still dispatching, a blocked node was visible only as a `⊘` glyph in the tree tab. On a live T3 run (two blocked nodes, ~13 still pending) there was no persistent statement of the "waiting on you" state anywhere. **Fix:** `app.js`'s center stream now renders a persistent amber `⊘ N BLOCKED` banner (styling mirrors the stalled banner, amber instead of red, no pulse) pinned above the feed whenever `snap.tree_counts.blocked > 0`, each blocked node id a link into its Gates tab (`openNode(n.id, "gates")`, per §10's "blocked → jump to the reason, not the row"). The snapshot fingerprint gains the blocked node ids (not just the count) so a same-count id change still re-renders. Source-text regression: `test_blocked_banner_renders_persistently` — the repo has no jsdom harness, `node --check` is the JS gate.
